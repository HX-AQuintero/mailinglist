...
var args struct {
	GrpcAdrr string `arg:"env:MAILINGLIST_GRPC_ADDR"`
}
...
func main() {
	...
	conn, err := grpc.Dial(args.GrpcAdrr, grpc.WithTransportCredentials(insecure.NewCredentials()))
	...
	// newEmail := createEmail(client, "test123@999.999")
	// newEmail.ConfirmedAt = 10000
	// updateEmail(client, newEmail)
	// getEmail(client, newEmail.Email)
	// deleteEmail(client, newEmail.Email)

	getEmailBatch(client, 5, 20)
	getEmailBatch(client, 5, 21)
	getEmailBatch(client, 5, 22)
}
